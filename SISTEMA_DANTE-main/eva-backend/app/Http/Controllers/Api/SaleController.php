    ...
    public function store(Request $request)
    {
        ...
        $result = DB::transaction(function () use ($request) {
            ...
            foreach ($request->items as $item) {
                ...
                $saleItemsData[] = ['product' => $product, 'quantity' => $item['quantity'], 'unit_price' => $product->price, 'subtotal' => $lineSubtotal];
            }
            ...
            if ($request->filled('client_id')) {
                ...
                if ($client) {
                    $invoiceType = ($client->document_type === 'ruc') ? 'factura' : 'boleta';
                }
            }
