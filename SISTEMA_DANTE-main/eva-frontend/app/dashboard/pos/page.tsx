...
export default function POSPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [catRes, prodRes] = await Promise.all([
          ...
          fetch("http://localhost:8000/api/products?active=1", { headers }) // Solo activos con receta
        ]);
  ...
  const processSale = async () => {
    ...
    try {
      ...
      // Save ticket data before clearing cart
      setTicketData({
        items: [...cart],
      ...
      const prodRes = await fetch("http://localhost:8000/api/products?active=1", { 
        headers: { "Authorization": `Bearer ${token}` } 
      });
  ...
  return (
    ...
    <div className="h-[calc(100vh-2rem)] flex gap-6 -m-4 relative overflow-hidden print:hidden">
      {/* Catálogo de Productos (Izquierda / Principal) */}
      ...
      {/* Overlay oscuro para móvil cuando el carrito está abierto */}
      {isCartOpen && (
        <div 
      ...
      {/* Carrito de Compras / Ticket (Derecha / Drawer) */}
      <div className={`
      ...
      `}>
        ...
        <div className="bg-mist p-6 border-t border-latte/30 space-y-4">
          ...
          >
            ...
            {isProcessing ? "Procesando..." : "Cobrar (Efectivo)"}
          </button>
        </div>
      </div>
    </div>
