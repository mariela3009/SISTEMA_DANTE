...
export default function InventarioPage() {
  ...
  const handleUpdateIngredient = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("eva_token");
      const res = await fetch(`http://localhost:8000/api/ingredients/${editIngredient.id}`, {
        method: "PUT",
  ...
  const handleDeleteIngredient = async (item: any) => {
    ...
    try {
      const token = localStorage.getItem("eva_token");
      const res = await fetch(`http://localhost:8000/api/ingredients/${item.id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  const handleEntrada = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("eva_token");
      const res = await fetch(`http://localhost:8000/api/inventory/entrada`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
        body: JSON.stringify({
